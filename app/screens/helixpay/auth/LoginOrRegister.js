...
export default class LoginOrRegister extends Base {
    ...
    validationSchema = yup.object({
        phoneNumber: __DEV__ ? yup.string().trim()
            .required(strings('error_empty_phone_number')) : yup.string().trim()
        ...
        return (
            <CBView style={{flex: 1, backgroundColor: theme.colors.primary}} define={'none'}>
                <CBTouchableWithoutFeedback style={{flex: 1}} define={'none'} onPress={this.onBlur}>
                    <CBView style={[{flex: 1, paddingVertical: 15, paddingHorizontal: 30}, {borderTopLeftRadius: 30, borderTopRightRadius: 30, marginTop: dimens.statusBar}]} define={'content'}>
